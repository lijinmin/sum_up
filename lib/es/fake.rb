class Es::Fake
	...
	def self.fake_name
		...
		name = (family_name[rand(0..490)] + family_name[rand(0..490)]).to_s
		return name
	end
	...
	def self.fake_email
		email = "#{rand(1..9)}#{rand(0..9)}#{rand(0..9)}#{rand(0..9)}#{rand(0..9)}#{rand(0..9)}#{rand(0..9)}#{rand(0..9)}@qq.com"
	end

	def self.fake_date
		date = "#{rand(2002..2017)}-#{"%02d" % rand(3..12)}-#{"%02d" % rand(1..30)}"
		return date
	end

	def self.fake_depart
		departs = ['人力资源部','企管部','技术部','生产部','销售部','财务部','后勤部','基建设备部','采购部']
		return departs[rand(0..8)]
	end
	
	def self.fake_sex
		return ['man','woman'][rand(0..1)]
	end
end